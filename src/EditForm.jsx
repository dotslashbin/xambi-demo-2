...
export function EditForm(props: EditFormProps) {
  ...
  /**
   * NOTE:
   * these below are the new code for the Pill component on entry type
   */
  const [pills, setPills] = useState([]);
  const [chosenPill, setChosenPill] = useState('');

  const [message, setMessage] = useState('');
  const addSelectedPillType = () => {

    setPills([...pills, chosenPill]);
    setMessage(`You added "${ chosenPill }" as a pill type...`)
  ...
  const handleDelete = (pillToRemove) => {
    setPills(pills.filter(type => type !== pillToRemove));
  }
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative lg:pt-5 text-left">
      <form className="space-y-8 divide-y divide-gray-200"
        onSubmit={(event) => {
          ...
          /**
           * New code for Pill ( Entry type )
           */
          // For Pill
          if(editEntry.type === EditEntryType.Pill) {
            entity['pills'] = pills
          }
            ...
            <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
              {
                props.editEntries.map((editEntry: EditEntry, index) => {
                          ...
                          <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            ...
                            <div className="col-span-6 sm:col-span-3">
                              ...
                              <div className="my-2">
                                ...
                                >
                                  <option
                                      selected={entity && entity[editEntry.attribute] && entity[editEntry.attribute]["country"] === "United States"}>United
                                    States
                                  </option>
                                  <option
                                      selected={entity && entity[editEntry.attribute] && entity[editEntry.attribute]["country"] === "Canada"}>Canada
                                  </option>
                                </select>
                              </div>
                            </div>
                  ...
                  } else if (editEntry.type === EditEntryType.Pill) {
                    return (
                        <>
                          <div className='form-field'>
                            <span>This for field is a simulation of what could be the sources for types</span>
                            <input type='text' value={chosenPill} name='enteredType' onChange={(event) => {
                              setChosenPill(event.target.value)
                            }}/>
                            <button type='button' className='edit-form-button' onClick={addSelectedPillType}>add pill type</button>
                          </div>
                          <div>
                            <PillList collection={pills} handleDelete={handleDelete}/>
                          </div>
                        </>
